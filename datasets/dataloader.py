...
from .datasets import FundusDataset
from .stratified_sampler import StratifiedSampler


def get_dataloader(config, data_dir, fold_df, val_fold_idx, split, transform=None):
    dataset = FundusDataset(config, data_dir, fold_df, val_fold_idx, split, transform=transform)
